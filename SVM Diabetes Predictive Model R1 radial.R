########################################################################
# Radial Support Vector Machine (SVM) Classification Results
...
for(i in 1:1000){
  ...
  svm.result <- svm(Unplanned.Visits.bin ~ ., data=Sanford.data.diabetes[-samp,], type = "C-classification",
                    kernel="radial", cost=1, gamma=0.007)
...
SVM.R.train.CR <- train.cell00
SVM.R.train.FA <- train.cell01
SVM.R.train.M <- train.cell10
SVM.R.train.H <- train.cell11
