########################################################################
# Extreme Gradient Boosting (XG Boost) Classification 
...
params <- list(max_depth = c(3, 6, 10, 15, 20, 50), eta = c(0.01, 0.5, 0.1, 0.3, 0.6, 0.9),
               nrounds = c(50, 100, 150, 200), gamma=c(0, 0.5, 1, 5, 10, 25), 
               min_child_weight = c(0, 1, 5, 10), colsample_bytree = c(0.1, 0.5, 1))
...
for(o in 1:5){ # Outer loop CV
  ...
  for(p in 1:nrow(params)){ # Parameter loop
    ...
    for(i in 1:5){ # Inner loop
      ...
      # Train model
      xgboost.result <- xgboost(data=as.matrix(Sanford.data.diabetes.outer[-inner.ind[[i]],-1]),
                                label=as.matrix(Sanford.data.diabetes.outer$Unplanned.Visits.bin[-inner.ind[[i]]]),
                                max_depth = params$max_depth[p], eta = params$eta[p], 
                                gamma = params$gamma[p], min_child_weight = params$min_child_weight[p],
                                nrounds = params$nrounds[p], colsample_bytree = params$colsample_bytree,
                                eval_metric = 'auc', objective = 'binary:logistic')
      
      # Get predicted results
      train.result <- round(predict(xgboost.result, newdata = as.matrix(Sanford.data.diabetes.outer[-inner.ind[[i]],-1])))
      test.result <- round(predict(xgboost.result, newdata = as.matrix(Sanford.data.diabetes.outer[inner.ind[[i]],-1])))
  ...
  xgboost.result <- xgboost(data=as.matrix(Sanford.data.diabetes.outer[-outer.ind[[o]],-1]),
                            label=as.matrix(Sanford.data.diabetes.outer$Unplanned.Visits.bin[-outer.ind[[o]]]),
                            max_depth = best.params$max_depth, eta = best.params$eta,
                            gamma = best.params$gamma, min_child_weight = best.params$min_child_weight,
                            colsample_bytree = best.params$colsample_bytree,
                            nrounds = best.params$nrounds, eval_metric = 'auc', objective = 'binary:logistic')
  # Get training and testing results
  train.result <- round(predict(xgboost.result, newdata = as.matrix(Sanford.data.diabetes.outer[-outer.ind[[o]],-1])))
  test.result <- round(predict(xgboost.result, newdata = as.matrix(Sanford.data.diabetes.outer[outer.ind[[o]],-1])))
  
  train.table <- prop.table(table(Sanford.data.diabetes.outer$Unplanned.Visits.bin[-outer.ind[[o]]], train.result),1)
  test.table <- prop.table(table(Sanford.data.diabetes.outer$Unplanned.Visits.bin[outer.ind[[o]]], test.result),1)
...
XGB.train <- train.table.all
XGB.test <- test.table.all

XGB.train.CR <- train.cell00  # CR = correct rejections
XGB.train.FA <- train.cell01  # FA = false alarms
XGB.train.M <- train.cell10   # M = misses
XGB.train.H <- train.cell11   # H = hits

XGB.test.CR <- test.cell00
XGB.test.FA <- test.cell01
XGB.test.M <- test.cell10
XGB.test.H <- test.cell11

# Get means and sd's of each cell
#  Training set
summary(XGB.train.CR)
sd(XGB.train.CR)
summary(XGB.train.FA)
sd(XGB.train.FA)
summary(XGB.train.M)
sd(XGB.train.M)
summary(XGB.train.H)
